 ...
 * All topics: warning (101–153) + regulatory (201–231) + right_of_way (301–310) + prohibitions (401–441) + public_transport (501–516) + information_guidance (601–640) + traffic_lights (701–729) + road_markings (801–821).
 */
export function shouldShowSignBadge(imageUrl?: string): boolean {
  const num = parseInt(extractSignNumber(imageUrl) ?? '');
  if (isNaN(num)) return false;
  return (num >= 101 && num <= 153) || (num >= 201 && num <= 231) || (num >= 301 && num <= 310) || (num >= 401 && num <= 441) || (num >= 501 && num <= 516) || (num >= 601 && num <= 640) || (num >= 701 && num <= 729) || (num >= 801 && num <= 821);
}

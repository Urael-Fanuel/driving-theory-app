...
const SIGN_NOTES = {
  ...
  807: `Sign 807 is a solid YELLOW line marking the edge of the drivable road surface (roadway boundary), used in places that have no curbstones — it substitutes for a curbstone at the right edge of the road. Source: Israel's official traffic sign board (State of Israel traffic sign board.pdf, page 66), verbatim: "807 קו רצוף בצבע צהוב: קו שפת הכביש, במקום שאין בו אבני שפה, נהג בצידו השמאלי של הקו ואל תעבור לצידו הימני אלא כדי למנוע תאונה או עיכוב לתנועה כשאתה נוסע לאט, או לשמירת הוראות כל דין."

This is NOT primarily a stopping/parking prohibition — it defines the right-hand boundary of the driving area. The instruction is: drive on the LEFT side of the line (i.e. stay within the driving lane), and it is absolutely forbidden to cross it or drive on its right side, EXCEPT: (1) to avoid an accident, (2) when YOU YOURSELF are driving slowly and would otherwise obstruct/delay the traffic behind you — you may move onto the right side to let faster traffic pass, or (3) when required to comply with the law.

⚠️ CORRECTED 2026-09-02 — a previous version of this note (and the app content generated from it) got exception (2) backwards, describing it as "you may cross the line to overtake a vehicle ahead of you that is driving slowly." That is the OPPOSITE of the real exception. The Hebrew is unambiguous: "עיכוב לתנועה כשאתה נוסע לאט" = "delay to traffic WHEN YOU [the driver following this sign] ARE DRIVING SLOWLY" — the exception exists so that a slow driver can pull onto the shoulder/right side to be overtaken, not so a driver can leave the lane to overtake someone else. Do NOT mention overtaking another vehicle at all for this sign.

Do NOT describe this sign as being about stopping or parking a vehicle — that is a different topic (see signs 817/818, which are about curbstone-painting parking rules, a different marking). The explanation and at least one question must state that this line marks the road's right edge/boundary and must drive to its left, and must list the three crossing exceptions using the corrected wording above.`,
...
⚠️ MANDATORY: המספר שמופיע על הלוחית **משתנה בהתאם למיקום בפועל** — 2500 (מטר) הוא רק הדוגמה הספציפית שבתמונה של האפליקציה הזו, לא ערך קבוע/רשמי של תמרור 916 עצמו. ההסבר חייב להבהיר בפירוש שזו לוחית עם מספר משתנה (המרחק בפועל עד אזור העבודה, בדוגמה זו 2500 מטר), לא לתאר את זה כאילו "916 = תמיד 2500 מטר".`,
  919: `תמרור 919 (מקבוצת תמרורים באתר עבודה) הוא סימון על פני הכביש — קו קטעים כפול בצבע כתום — ומשמעותו "נתיב מתחלף": נתיב שאסור להיכנס אליו אלא אם כן מוצג מעליו אות המתיר זאת (למשל חץ ירוק כלפי מטה = הנתיב פתוח לנסיעה; איקס אדום = הנתיב סגור). זהו סימון זמני באתר עבודה, ואינו קשור בכלל לתחבורה ציבורית.
מאומת מ-noeg.co.il. חשוב מאוד: התוכן הקיים כרגע שגוי — הוא מתאר "נתיב ייעודי לאוטובוסים/תחבורה ציבורית", אבל בתמונה יש רק שני קווים מקווקווים כתומים, בלי שום סמל אוטובוס. יש להחליף את המשמעות כולה ל"נתיב מתחלף" (כיוון/היתר הנסיעה בנתיב יכול להתחלף, בהתאם לאות שמוצג מעליו — לא להזכיר מספרי תמרורים בהסבר, רק לתאר את החץ הירוק/האיקס האדום במילים).`,
...
מקור: המשתמש (בעל האפליקציה) אימת את זה ישירות. חשוב מאוד: התוכן הקיים כרגע שגוי — הוא מתאר את זה כ"נתיב ייעודי לפנייה שמאלה/U-turn" בלבד, אבל זו רק אחת מכמה אפשרויות; הסימון הכללי הוא "שינוי ייעוד נתיב", לא ספציפית פנייה שמאלה. זה גם מסביר למה בתמונה יש כמה סוגי חצים שונים (שמאלה, מעוקל, כפול, ישר) — כי זה סימון כללי לייעוד-נתיב-משתנה, לא תמיד אותו ייעוד. ההסבר וכל שאלה/תשובה חייבים לתאר את הרעיון הכללי (נתיב שמשנה ייעוד, נהיגה לפי הסימון בפועל באותו מקום) ולא לנעול על "פנייה שמאלה/U-turn" בלבד.`,
  924: `תמרור 924 (מקבוצת תמרורים באתר עבודה) הוא סימון על פני הכביש בצבע כתום, המורכב מקו קטעים (מקווקו). מיקומו הנפוץ הוא בתוך צמתים הנמצאים בתחומי אתר עבודה בדרך. הוראת הנסיעה: על הנהג לנסוע במקביל לקו הקטעים בעת הפנייה בצומת. פניית פרסה (U-turn) מותרת בעת היתקלות בסימון הזה, אלא אם כן היא נאסרה במפורש על ידי תמרור אחר.
מקור: המשתמש (בעל האפליקציה) אימת את זה ישירות. חשוב מאוד: התוכן הקיים כרגע שגוי לגמרי — הוא מתאר את זה כ"כיכר תנועה (ראונדאבאוט) מתקרבת", כולל שימוש בתעתיק פונטי אנגלי אסור ("ራውንድአባውት"). זו אינה כיכר תנועה בכלל — זהו סימון קווי-קטעים כתום בתוך צומת רגיל (לא עגול) שמנחה איך לנסוע בפנייה בצומת, ומבהיר שפניית פרסה מותרת שם אלא אם נאסרה במפורש. אסור להזכיר כיכר תנועה, נסיעה נגד כיוון השעון, או מתן קדימות לתנועה במעגל — אלה כולם שייכים לתמרור אחר לגמרי. ההסבר וכל שאלה/תשובה חייבים לתאר: (1) קו קטעים כתום בתוך צומת באתר עבודה, (2) נסיעה מקבילה לקו בעת פנייה, (3) פרסה מותרת אלא אם נאסרה על ידי תמרור אחר.`,
  925: `תמרור 925 (מקבוצת תמרורים באתר עבודה) הוא סימון על פני הדרך המציין קו עצירה בצבע כתום, שלם (לא מקווקו), לרוחב נתיב הנסיעה. משמעותו: המקום שבו על הרכב (או בעל חיים) לעצור — לפני הקו ובסמוך לו — כאשר יש חובת עצירה. מוצב לרוב באזורים של עבודות בדרך או בשינויים זמניים בהסדרי התנועה.
מקור: המשתמש (בעל האפליקציה) אימת את זה ישירות. חשוב מאוד: התוכן הקיים כרגע שגוי לגמרי — הוא מתאר "איסור עצירה מוחלט" (כמו שלט no-stopping), אבל זו טעות: זהו קו עצירה (stop line), לא איסור עצירה. הכיוון הפוך למעשה — התמרור לא אוסר עצירה, הוא **מחייב** עצירה במקום מסוים כשיש חובת עצירה (למשל לפני איתות/הנחיה בעבודות בדרך). ההסבר וכל שאלה/תשובה חייבים לתאר: (1) קו כתום שלם לרוחב הנתיב, (2) חובה לעצור לפניו ובסמוך אליו כשנדרשת עצירה, (3) לא לתאר את זה כאיסור עצירה כללי.`,
  926: `תמרור 926 (מקבוצת תמרורים באתר עבודה) הוא סימון על פני הדרך (בצבע כתום, לעיתים עם חצים לבנים) המורה על כיווני הנסיעה המותרים בנתיב לקראת צומת. חובה להיכנס לצומת ולעבור בו אך ורק לפי כיוון החץ שסומן בנתיב שממנו נכנסים. משמעות והנחיות: (1) ציות לחצים — יש להשתלב בנתיב המתאים מראש ולנסוע רק לפי הכיוון המסומן (ישר, פנייה שמאלה, או פנייה ימינה). (2) צבע כתום מעיד לרוב על שינוי זמני בהסדר התנועה (כמו באתרי עבודות בדרך); חץ לבן (כשמופיע לצד הכתומים) מציין את כיוון הנסיעה הרגיל/הקבוע. (3) איסור סטייה — אסור להמשיך בישר מנתיב המסומן לפנייה בלבד, ואסור לפנות מנתיב המסומן לנסיעה ישר בלבד.
⚠️ חשוב לניסוח: אין בתמונה שום צומת מצויר בפועל — רק קבוצות חצים על הכביש. אל תתאר את זה כאילו התמרור "בתוך צומת" או "מציג צומת" — זה סימון חצים על הנתיב שמנחה לקראת צומת קרוב, לא תיאור חזותי של הצומת עצמו.
⚠️ MANDATORY — אל תשמיט את זה שוב (זה נשמט בניסיון קודם): explanation_amharic **חייב** להזכיר את שני צבעי החץ ואת המשמעות של כל אחד — חץ כתום = כיוון הנסיעה הזמני/הנוכחי (למשל בזמן עבודות בדרך), חץ לבן (כשמופיע) = כיוון הנסיעה הרגיל/הקבוע. וגם **לפחות שאלה אחת** מתוך ה-3 חייבת לבדוק במפורש את ההבדל בין המשמעות של חץ כתום לחץ לבן — לא רק "למה החץ כתום", אלא ההשוואה בין השניים.
מאומת: המשתמש (בעל האפליקציה) אישר וניסח את התוכן המדויק הזה ישירות, אחרי שהגרסה הקודמת שלי טעתה בניסוח "בתוך צומת".`,
  927: `תמרור 927 (מקבוצת תמרורים באתר עבודה) הוא סימון על פני הכביש המציין שנתיב הנסיעה הנוכחי עומד להסתיים. מורכב מחצים אלכסוניים בצבע כתום המסומנים על הכביש. הוראה לנהג: יש להשתלב בבטחה אל הנתיב הסמוך, בהתאם לכיוון שאליו מורים החצים. נמצא בדרך כלל בתוך או בסמוך לאתרי עבודה בכביש שבהם מתבצעות חסימות או שינויים זמניים בנתיבי הנסיעה.
מקור: המשתמש (בעל האפליקציה) אימת את זה ישירות. חשוב מאוד: התוכן הקיים כרגע שגוי לגמרי — הוא מתאר "פסי האטה/רעד" (rumble strips, כולל תעתיק אנגלי אסור), אבל זה לא קשור לרעד/קול בכלל. המשמעות האמיתית: הנתיב הנוכחי מסתיים/נחסם, וחובה לעבור לנתיב הסמוך בכיוון החצים. אסור להזכיר רעד, קול, פסי האטה, או הפחתת מהירות בגלל מרקם הכביש — ההסבר וכל שאלה/תשובה חייבים לתאר במפורש: (1) הנתיב הנוכחי מסתיים/נחסם, (2) יש להשתלב לנתיב הסמוך בכיוון החצים, (3) הקשר לאתר עבודה זמני.`,
  928: `תמרור 928 (מקבוצת תמרורים באתר עבודה) הוא סימון על פני הדרך בצבע כתום המציין אי תנועה זמני, בצורות שונות (למשל קווים אלכסוניים בתוך משולש, או צורה מוארכת/אליפסה עם קווים אלכסוניים). אסור לנהגים לעבור עליו, לעצור עליו, או לחנות עליו. כוחו יפה במקום הסימון עצמו, בדרך כלל באתרי עבודה בדרך.
משמעות והנחיות: (1) אי תנועה כתום — מסומן על הכביש בקווים אלכסוניים בתוך צורה הנדסית (משולש או צורה מוארכת). (2) איסור מעבר — אסור לחצות או לעלות על הסימון עם הרכב. (3) איסור עצירה וחניה — אסור בהחלט לעצור או להחנות רכב על גבי השטח המסומן. (4) הקשר — מופיע בעיקר באזורים של עבודות בכביש להכוונת תנועה זמנית.
מקור: המשתמש (בעל האפליקציה) אימת את זה ישירות ואושר ויזואלית מהתמונה (משולש וצורה מוארכת עם קווים אלכסוניים כתומים). חשוב מאוד: התוכן הקיים כרגע שגוי לגמרי — הוא מתאר "אזהרת רוח צד חזקה", שאין לה שום קשר לתמונה או למשמעות האמיתית. אסור להזכיר רוח, מזג אוויר, או אחיזת הגה — ההסבר וכל שאלה/תשובה חייבים לעסוק אך ורק באי התנועה הכתום ובאיסור המעבר/עצירה/חניה עליו.
⚠️ MANDATORY — דיוק קריטי: "אי תנועה" הוא שטח מסומן קטן וממוקם על הכביש (כמו אי תנועה/משטח חסום קטן, בגודל של הצורה המצוירת בלבד — משולש או אליפסה), **לא סגירת כביש/דרך שלמה**. אל תשתמש במילים שמשמעותן "סגירת כביש/דרך" (כמו "የመንገድ መዘጋት") — זה מגזים ומטעה. השתמש בניסוח שמבהיר שזה שטח קטן ומסומן שאסור לעבור/לעצור/לחנות עליו, בתוך הכביש הפעיל, לא חסימה של הכביש כולו.
⚠️ MANDATORY — explanation_amharic חייב לפרט את שלושת האיסורים בנפרד ובבירור, לא כמשפט מכליל אחד: (1) אסור לחצות או לעבור על השטח המסומן עם גוף הרכב, (2) אסור לעצור עליו, (3) אסור לחנות עליו. שלושתם חייבים להופיע בבירור, לא רק "ማለፍ" כללי — יש להשתמש בניסוח שמבהיר שמדובר בחצייה/מעבר עם הרכב עצמו על גבי השטח המסומן, בנפרד מהאיסור על עצירה ובנפרד מהאיסור על חניה.
⚠️ MANDATORY: אסור להשתמש במילה "እንዳይነዱ" בשום מקום (בעיית הגייה באודיו, ראה כלל גלובלי) — יש למצוא ניסוח אחר לגמרי לאיסור החצייה/מעבר עם הרכב.
⚠️ MANDATORY EXACT STRUCTURE — previous attempts were too vague (e.g. "መጠቀም የማይፈቀድበትን ቦታ" = "a place where use is not permitted", which does not say WHAT is forbidden). explanation_amharic must contain THREE SEPARATE, EXPLICIT clauses, in this exact logical order, each naming the specific forbidden action — not one vague summary word like "use" or "ክልከላ" alone:
  (1) forbidden to CROSS/PASS THROUGH the marked area (חצייה/מעבר דרכו),
  (2) forbidden to STOP on it even briefly/for a short time (עצירה קצרה),
  (3) forbidden to STOP on it for a long time / park (עצירה ארוכה / חניה).
Do not compress these into one generic phrase — a listener who cannot read must hear all three distinct actions named clearly. At least one question must ask specifically about this list of three forbidden actions.
⚠️ MANDATORY — confirmed twice by the app owner after listening to the audio: do NOT use the word for "vehicle" (ተሽከርካሪ) anywhere in clause (1) at all — it is unnecessary and sounds confusing/ambiguous when read aloud. Describe clause (1) purely in terms of the ROAD/LANE/marked area itself (e.g. "ማለፍ/መሻገር" applied to the road/lane/marked spot) — do not mention the vehicle as the subject of the crossing action. "ተሽከርካሪ" may still appear elsewhere (e.g. clauses 2-3 about stopping/parking a vehicle) — the restriction is specifically on clause (1)'s crossing/passing description.
⚠️ MANDATORY: do NOT use "ፓርክ ማድረግ" (a phonetic transliteration of the English word "park") anywhere — this violates the standing no-transliteration rule. Use only the proper Amharic verb "ማቆም" for parking/leaving a vehicle standing, exactly as already used for the short-stop clause. Also double-check every answer choice for accidental word repetition (e.g. do not write "ማቆም ወይም ማቆም ማቆም") before returning the JSON — proofread each answer text once before finalizing.
⚠️ MANDATORY — ABSOLUTE: the word/concept "road closure" (e.g. "የመንገድ መዘጋት", "መዘጋት") must NEVER appear ANYWHERE in this sign's content, for ANY reason — not as the sign's meaning, not as a wrong-answer description, not even inside a wrong-answer explanation that negates it (e.g. do NOT write anything meaning "this does not mean the road is closed"). If a wrong answer needs to describe a road-closure-like misconception, describe it some other way (e.g. contrast with "a small marked spot" instead of explicitly naming "road closure"), or omit that contrast entirely rather than using this concept at all.`,
  929: `תמרור 929 (מקבוצת תמרורים באתר עבודה) הוא תמרור מחסום, שפירושו שהדרך חסומה (בפני כלי רכב או עובדים). זהו אותו רעיון כמו תמרור 403 (מחסום כללי), רק בגרסת אתר עבודה. משמעות והנחיה נלווית: אם הוצב לצד התמרור תמרור נוסף שמורה על כיוון נסיעה, מותר להתקדם אך ורק לכיוון המצוין באותו תמרור נלווה. אם אין תמרור כיוון נלווה, הדרך פשוט חסומה ואין להמשיך.
מקור: המשתמש (בעל האפליקציה) אימת את זה ישירות, בהתבסס על אותו עקרון שכבר אושר לתמרור 403. חשוב מאוד: התוכן הקיים כרגע מנוסח בעמימות — הוא מערבב "מכשול, כביש סגור, או אזור עבודה מסוכן" כשלוש אפשרויות מקבילות, במקום לקבוע בבירור שזה **מחסום = הדרך חסומה**, עם ההנחיה הנלווית על תמרור כיוון אם קיים. ההסבר וכל שאלה/תשובה חייבים לתאר במפורש: (1) הדרך חסומה, (2) אם יש תמרור כיוון נלווה — חובה לציית לו ולהתקדם רק לכיוון שהוא מורה.`,
  930: `תמרור 930 (מקבוצת תמרורים באתר עבודה) הוא תמרור אזהרה והכוונה שמשמעותו "עמודי הכוונה בצד הכביש" — עמודים המוצבים בצד הכביש עם קווים אלכסוניים בצבע כתום-שחור, כאשר הקווים מוטים כלפי מטה לכיוון מרכז הנתיב הסמוך והפתוח. מטרתו: להראות לנהג את תוואי הנסיעה הבטוח סביב אזור עבודות/מכשול בכביש, ולהזהיר מפני מכשולים או שינויים זמניים בנתיבי הנסיעה.
מקור: המשתמש (בעל האפליקציה) אימת את זה ישירות ואושר ויזואלית מהתמונה (שני עמודים אנכיים עם פסים אלכסוניים כתום-שחור). חשוב מאוד: התוכן הקיים כרגע כללי מדי — הוא מתאר "מכשול/אזור עבודה/סכנה" בעמימות, בלי לתאר בכלל את העמודים הפיזיים בצד הכביש ואת כיוון הקווים כהכוונה לנתיב הפתוח. ההסבר וכל שאלה/תשובה חייבים לתאר במפורש: (1) אלה עמודי הכוונה בצד הכביש, (2) הקווים האלכסוניים עליהם מטים כלפי הנתיב הפתוח/הסמוך, (3) התפקיד הוא הכוונת הנהג סביב המכשול/העבודה בבטחה, לא רק אזהרה כללית.`,
  931: `תמרור 931 (מקבוצת תמרורים באתר עבודה) הוא קונוס (חרוט) להכוונה או לסימון מכשול בכביש. הכלל: יש לנסוע בהתאם למיקום שבו הוצבו הקונוסים, ואסור לדרוס/לעבור מעל המכשול המסומן על ידי הקונוס. תוקפו במקום שבו הוצב בפועל, בדרך כלל באתר עבודה בדרך.
מקור: אושר ויזואלית מהתמונה (קונוס כתום-שחור קלאסי על בסיס משולש/מעוין). חשוב מאוד: התוכן הקיים כרגע כללי מדי וכולל ניחוש מיותר — הוא מתאר "סכנה זמנית, אזור עבודה, או שינוי כיוון תנועה" בעמימות, בלי לומר בכלל שזה קונוס, ובלי לציין את הכלל הספציפי (איסור לדרוס/לעבור מעליו, נסיעה לפי מיקום הקונוסים). ההסבר וכל שאלה/תשובה חייבים לתאר במפורש: (1) זהו קונוס להכוונה/סימון מכשול, (2) יש לנסוע לפי מיקום הקונוסים, (3) אסור לדרוס/לעבור מעל המכשול המסומן.`,
  932: `תמרור 932 (מקבוצת תמרורים באתר עבודה) הוא מנורה מהבהבת (אורית) בצבע כתום, המשמשת לאיתות, הודעה ואזהרה. תוקפו במקום שבו הוצב בפועל באתר עבודה בדרך. משמש לאזהרה ייחודית באתרי עבודה, בעיקר במקומות שבהם עוברים משתמשי דרך שונים (נהגים והולכי רגל). מוצב בדרך כלל מחובר לתמרור 901 (בפינה העליונה) או לתמרור 902 (בחלקו העליון), בכניסה לקטע כביש באתר עבודה או ביציאה ממנו.
מקור: מקור עם התאמה ישירה למספר התמרור, ותואם ויזואלית לתמונה (עיגול כתום פשוט, המייצג נורית מהבהבת). חשוב מאוד: התוכן הקיים כרגע שגוי לגמרי — הוא מתאר "ציות להוראות שוטר/מפקח תנועה", שאין לו שום קשר לעיגול הכתום הפשוט בתמונה. אסור להזכיר שוטר, מפקח תנועה, או ציות להוראות אדם — ההסבר וכל שאלה/תשובה חייבים לעסוק אך ורק במנורה המהבהבת הכתומה ובתפקידה כאזהרה/איתות באתר עבודה.`,
  933: `תמרור 933 (מקבוצת תמרורים באתר עבודה) הוא לוח נייד מהבהב, המשמש לאיתות, אזהרה והכוונה. תוקפו במקום שבו הוצב בפועל באתר עבודה בדרך. הלוח עצמו מציין נוכחות אתר עבודה/מכשול, אך את כיוון הנסיעה בפועל קובע תמרור/חץ נוסף שמוצב יחד איתו (למשל חץ בתוך עיגול כחול) — וזה יכול להשתנות ממקרה למקרה (פעם שמאלה, פעם ימינה, וכו'). לכן הכלל החשוב הוא: יש לציית לכיוון שמורה התמרור הנוסף שמוצג יחד עם הלוח המהבהב, לא כלל קבוע של "תמיד שמאלה". בתמונה של התמרור הספציפי הזה מוצג חץ לבן בתוך עיגול כחול הפונה שמאלה-מטה, כך שבמקרה הזה יש לעבור בצד שמאל של המכשול/אזור העבודה — אבל זו דוגמה אחת, לא הכלל הכללי.
מקור: מקור עם התאמה ישירה למספר התמרור (l4me.org.il, מאושש גם ב-noeg.co.il וב-Wikibooks עברית: "לוח נייד מהבהב: איתות, אזהרה והכוונה"), ותואם ויזואלית לתמונה; ההבהרה על התמרור הנוסף המשתנה אושרה ישירות על ידי בעל האפליקציה. חשוב מאוד: אין לבלבל עם תמרור 934, שהוא אותו סוג לוח נייד מהבהב אך מסמן "הכביש סגור" — אסור להזכיר סגירת כביש בתמרור זה בכלל. ⚠️ MANDATORY: ההסבר וכל שאלה/תשובה חייבים להבהיר בבירור ששמאל הוא מה שמוצג בתמונה הספציפית הזו, אך הכלל הכללי הוא לציית לכיוון שמורה התמרור/חץ הנוסף (שיכול להצביע לכל כיוון) — אסור לנסח את זה כאילו לוח 933 תמיד מורה לעבור שמאלה.`,
  934: `תמרור 934 (מקבוצת תמרורים באתר עבודה) הוא לוח נייד מהבהב המסמל "הכביש סגור". התמרור מוצב בכניסה לקטע כביש החסום לחלוטין לתנועה. זהו תמרור המציג הודעה ברורה שהכביש סגור. הוא מחייב את הנהגים לא להמשיך בנסיעה בנתיב/בכביש הזה, ובמקום זה לחפש דרך חלופית ולנסוע בה.
מקור: מקור עם התאמה ישירה למספר התמרור (noeg.co.il/tamrurs/9/308/view.html: "לוח נייד מהבהב: הכביש סגור"), ותואם ויזואלית לתמונה (X כתום גדול על רקע שחור, באותה מסגרת עם נקודות כתומות/לבנות מהבהבות כמו תמרור 933). חשוב מאוד: התוכן הקיים כרגע שגוי לגמרי — הוא מתאר "מעבר רכבת לא מוגן/מסילת רכבת", שאין לו שום קשר לתמרור הזה. אסור להזכיר רכבת, מסילה, או מחסום רכבת בכלל. גם אסור לבלבל עם תמרור 933 (אותו סוג לוח נייד מהבהב, אך שם מכוון לכיוון חלופי כמו שמאל/ימין — כאן ה-X אומר שאסור להמשיך בנתיב/בכביש הזה בכלל, וצריך למצוא דרך חלופית). ההסבר וכל שאלה/תשובה חייבים לתאר במפורש: (1) זהו לוח נייד מהבהב, (2) ה-X מסמל שהכביש/הנתיב סגור לחלוטין, (3) יש לחפש דרך חלופית ולא להמשיך בנתיב הזה. ⚠️ MANDATORY — ABSOLUTE: המילה/המושג "רכבת"/"מסילת רכבת"/"מעבר רכבת" אסורים לחלוטין בכל מקום בתוכן הזה — לא בהסבר, לא בשאלה, ולא אפילו כתשובה שגויה/הסחת דעת. אם נדרשת הסחת דעת שגויה לשאלה, השתמש ברעיון אחר (למשל "כביש בתיקון קל, נא להאט" או "פנייה שמאלה/ימינה") ולא ברכבת בשום צורה.`,
  935: `תמרור 935 (מקבוצת תמרורים באתר עבודה, האחרון בקבוצה) הוא מפרץ לעצירה במצב חירום באתר עבודה — שטח ייעודי בצד הכביש שבו נהג שנקלע למצב חירום (תקלה ברכב, בעיה בריאותית, וכדומה) יכול לעצור באופן בטוח, מחוץ לנתיב הנסיעה הרגיל, מבלי לחסום את התנועה.
מקור: שני מקורות עצמאיים עם התאמה ישירה למספר התמרור (traffic-safety.co.il ו-theorytest.co.il), שניהם בדיוק באותו ניסוח: "מפרץ לעצירה במצב חירום באתר עבודה". חשוב מאוד: התוכן הקיים כרגע שגוי לגמרי — הוא מתאר "הכביש מצטמצם משמאל" (צמצום נתיב), שאין לו שום קשר למפרץ עצירת חירום. אסור להזכיר צמצום נתיב/כביש מצטמצם/התכווצות הכביש בכלל. ההסבר וכל שאלה/תשובה חייבים לתאר במפורש: (1) זהו מפרץ (שטח) ייעודי לעצירה במצב חירום, (2) הוא נמצא בצד הכביש, מחוץ לנתיב הנסיעה הרגיל, (3) הוא מיועד לשימוש רק במצב חירום אמיתי (לא לעצירה/חנייה רגילה סתם כך).`,
};
...
3. questions: Exactly 3 questions that test understanding of THIS specific sign.
   ...
   - NEVER include a Hebrew word anywhere in name_amharic, explanation_amharic, or any
     question/answer/explanation — not even in parentheses as a translation gloss (e.g. do NOT write
     something like "መገናኛ መንገድ (צומת)"). This content is for Amharic speakers; a stray Hebrew word is
     unreadable to most of them and meaningless when read aloud by TTS. Write only in Amharic.
   - NEVER use the word "እንዳይነዱ" anywhere, in any form — confirmed by the app owner that the
     text-to-speech voice mispronounces this specific word. Use a different verb entirely to express
     "must not drive/cross" — never this exact word, permanently, in every sign from now on.

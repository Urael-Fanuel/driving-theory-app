/**
 * Generates number_1.mp3 through number_4.mp3 (Amharic numbers 1-4)
...
const NUMBERS = [
  { file: 'number_1.mp3', text: 'አንድ' },
  { file: 'number_2.mp3', text: 'ሁለት' },
  { file: 'number_3.mp3', text: 'ሦስት' },
  { file: 'number_4.mp3', text: 'አራት' },
];
...
for (const { file, text } of NUMBERS) {
  console.log(`🎵 Generating ${file} ("${text}")...`);
  const audio = await googleTTS(text);
  const outputPath = path.join(ROOT, 'assets', 'audio', file);
  ...
  if (error) {
    ...
    console.log(`  ✅ Uploaded to Supabase: ${file}`);
  }
}

console.log('\n🎉 Done! All 4 number files generated and uploaded.');
